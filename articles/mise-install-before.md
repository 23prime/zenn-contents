...
**mise [v2026.4.22](https://github.com/jdx/mise/releases/tag/v2026.4.22) にて、`install_before` は `minimum_release_age` にリネームされました。**
`install_before` はエイリアスとして引き続き動作しますが、非推奨とされています。

最新の mise をお使いの場合は `minimum_release_age` へ移行すると良いでしょう。

この記事では引き続き `install_before` を使用していますが、適宜 `minimum_release_age` に読み替えてください。
:::

皆さんは mise 使ってますか？

便利ですよね mise。
今や開発ツール（環境）管理はかなり mise に支配され、欠かせないものとなっています。

今回は、そんな mise で Minimum Release Age 的なものを設定する方法をご紹介します。

## 👤 対象読者

- mise で開発ツールを管理している開発者
- CI でのツール導入タイミングを少しでも安全にしたい人

## 🕵️ 背景：OSS とサプライチェーン攻撃

近年、OSS のエコシステムを狙ったサプライチェーン攻撃が増加しています。IPA（情報処理推進機構）の「[情報セキュリティ10大脅威 2026](https://www.ipa.go.jp/security/10threats/10threats2026.html)」では、「サプライチェーンや委託先を狙った攻撃」が組織向け脅威の2位にランクインしており、継続的な脅威として認識されています。
...
| ツール単位設定 | v2026.4.1 |
| `minimum_release_age`（`install_before` のリネーム） | v2026.4.22 |
